...
[PcdsFeatureFlag.common]
  ...
  # Gunyah: disable RemapUnusedMemoryNx — firmware runs from LEND'd RAM,
  # and the NX remap causes permission faults in the FD region.
...
[PcdsFixedAtBuild.common]
  ...
  # Gunyah: stack must be AFTER the 3MB FD loaded at PcdFdBaseAddress
...
!include NetworkPkg/NetworkFixedPcds.dsc.inc
  ...
  # System Memory Base -- Gunyah places guest RAM at 0x8000_0000
  gArmTokenSpaceGuid.PcdSystemMemoryBase|0x80000000

  # Gunyah: DTB placed at mem_base + 4MB (after FD and stack, before UEFI region end)
  ...
  #
  # Gunyah: disable strict memory protection — firmware FD lives in LEND'd
  # RAM (not flash), so NX policies on the FD region cause permission faults.
  #
...
[PcdsDynamicHii]
  # Gunyah: Force DTB mode so that restricted-dma-pool is preserved for
  # LEND/SHARE DMA isolation.  ACPI tables for Windows are separately
...
!endif
  ...
  # Gunyah: removed NvVarStoreFormattedLib (no NOR flash, using EmuVariableFvb)
  ...
  #
  # Gunyah: RAM-based variable store (NOR flash at 0x0 is not accessible)
  #
  OvmfPkg/EmuVariableFvbRuntimeDxe/Fvb.inf

  #
  # Gunyah: IoMmu driver for DMA bounce buffers through SHARE'd memory
  #
  ArmVirtPkg/GunyahIoMmuDxe/GunyahIoMmuDxe.inf
...
[Components.AARCH64]
  ...
  #
  # SMBIOS — static platform tables (ported from edk2-gunyah).
  ...
  # This is the crosvm/edk2-gunyah approach — ACPI tables come from
  # the DTB (not QEMU fw_cfg), so they only describe Gunyah-visible devices.
  # fw_cfg AcpiPlatformDxe is removed to avoid installing QEMU's ACPI tables
  # which reference devices Gunyah doesn't pass through.
  #
  ArmVirtPkg/KvmtoolCfgMgrDxe/ConfigurationManagerDxe.inf {
    <LibraryClasses>
      ...
      DynamicPlatRepoLib|DynamicTablesPkg/Library/Common/DynamicPlatRepoLib/DynamicPlatRepoLib.inf
    #
    # Override depex: the .inf requires gEdkiiPlatformHasAcpiGuid which is
    # only installed when PcdForceNoAcpi=FALSE.  Under Gunyah we need
    # PcdForceNoAcpi=TRUE (for restricted-dma-pool / DTB mode) but still
    # want ACPI tables generated for Windows.  Remove the ACPI gate.
    #
    <Depex>
      TRUE
  }
  ...
  #
  # Gunyah restricted DMA pool ACPI SSDT — generates \_SB_.RDPA / \_SB_.RDPS
  # from the DTB restricted-dma-pool node.
  #
  ArmVirtPkg/GunyahRestrictedDmaPoolAcpiDxe/GunyahRestrictedDmaPoolAcpiDxe.inf
